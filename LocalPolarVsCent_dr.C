...
void LocalPolarVsCent_dr(){
    TFile *inputFirst = new TFile("../output/output_PxVsCent_forPsi2_ConstSine_14p5.root", "read");
    TFile *inputSecond = new TFile("../output/output_PxVsCent_forPsi3_ConstSine_14p5.root", "read");
    TFile *inputThird = new TFile("../output/output_PzVsCent_forPsi2_14p5.root", "read");
    TFile *inputFourth = new TFile("../output/output_PzVsCent_forPsi3_14p5.root", "read");

    TGraphErrors *PxVsCentFirst = (TGraphErrors*)inputFirst->Get("PxVsCent_Tog_0");
    TGraphErrors *PxVsCentSecond = (TGraphErrors*)inputSecond->Get("PxVsCent_Tog_0");
    TGraphErrors *PxVsCentThird = (TGraphErrors*)inputThird->Get("PzVsCent_Tog_0");
    TGraphErrors *PxVsCentFourth = (TGraphErrors*)inputFourth->Get("PzVsCent_Tog_0");
    ...
    PxVsCent_Third_Fourth->GetYaxis()->SetTitle("<P_{z}sin(n(#phi - #Psi_{n}))> %");
    ...
    PxVsCent_Third_Fourth->SetMaximum(1.6);
    PxVsCent_Third_Fourth->SetMinimum(-0.6);

    TLegend *leg_Third_Fourth = new TLegend();

    leg_Third_Fourth->AddEntry((TObject*) 0, "P_{z} of #Lambda + #bar{#Lambda}, #Psi_{n,ew} used", "");
    ...
    PxVsCent_First_Second->GetYaxis()->SetTitle("<P_{x}sin(n(#phi - #Psi_{n}))> %");
    ...
    PxVsCent_First_Second->SetMaximum(0.6);
    PxVsCent_First_Second->SetMinimum(-1.8);

    TLegend *leg_first_second = new TLegend();

    leg_first_second->AddEntry((TObject*) 0, "P_{x} of #Lambda + #bar{#Lambda}, #Psi_{n,ew} used", "");
